...
def _resharding_ops(uri) -> int:
    return len(get_collection_cluster_info(uri, COLLECTION).get("resharding_operations") or [])


def _replica_state(uri, shard_id, peer_id):
    return next(
        (r["state"] for r in all_replicas(get_collection_cluster_info(uri, COLLECTION))
...
def test_scale_down_revert_crash_orphans_resharding_state(tmp_path: pathlib.Path):
    ...
    assert_http_ok(start_resharding(peer_uris[0], COLLECTION, direction="down", shard_key=None))
    ...
    surviving_peers = [r["peer_id"] for r in all_replicas(info) if r["shard_id"] == surviving]
    from_peer, receiver_peer = next((f, t) for f in target_peers for t in surviving_peers if f != t)

    assert_http_ok(requests.post(f"{peer_uris[0]}/collections/{COLLECTION}/cluster", json={
        "replicate_shard": {
            "from_peer_id": from_peer, "to_peer_id": receiver_peer,
            "shard_id": target, "to_shard_id": surviving,
            "method": "resharding_stream_records",
        }
    }))
    wait_for_collection_shard_transfers_count(peer_uris[0], COLLECTION, 0)
    wait_for(lambda: _replica_state(peer_uris[0], surviving, receiver_peer) == "ReshardingScaleDown")
    ...
    def drive_failing_updates():
        while not stop.is_set():
            try:
                upsert_random_points(survivor_uri, num=100, collection_name=COLLECTION,
                                     fail_on_error=False, ordering="weak", timeout=5)
    ...
    for idx in (victim_idx, receiver_idx):
        peer_uris[idx] = start_peer(peer_dirs[idx], f"peer_restart_{idx}.log",
                                    survivor_uri, port=peer_procs[idx].p2p_port)
        wait_for_peer_online(peer_uris[idx], path="/healthz", wait_for_timeout=60)

    stuck = {}
    for uri in peer_uris:
        try:
            wait_for_collection_resharding_operations_count(uri, COLLECTION, 0, wait_for_timeout=10)
        except Exception:
