...
async fn drain_snapshot(
    ...
    let description = tokio::time::timeout(SNAPSHOT_HANG_TIMEOUT, handle)
        ...
        .expect("background snapshot task panicked")
...
mod tests {
    ...
    /// Only `disable_optimizer`, `restart_probability`, `op_num`, and `disable_snapshots` vary
    /// between tests.
    ///
    ...
    async fn harness_no_restarts() {
        smoke("harness_no_restarts", false, 0.0, OP_NUM, SNAPSHOTS_OFF).await;
    }
    ...
    async fn harness_no_optimizer() {
        smoke("harness_no_optimizer", true, 0.002, OP_NUM, SNAPSHOTS_OFF).await;
    }
    ...
    async fn harness() {
        ...
        smoke("harness", false, 0.002, OP_NUM / 4, SNAPSHOTS_OFF).await;
    }
